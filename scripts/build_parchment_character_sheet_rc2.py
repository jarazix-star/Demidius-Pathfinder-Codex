...
def draw_first_page_identity_banner(page: Image.Image, page_number: int) -> None:
    """Replace the source identity strip with a full-width campaign banner."""

    if page_number != 1:
        return

    draw = ImageDraw.Draw(page)
    left, top, right, bottom = 72, 142, PAGE_WIDTH - 72, 430

    # A broad parchment inset covers the original Hero Lab identity strip while
    # preserving the statistics that begin immediately below it.
    draw.rounded_rectangle(
        (left, top, right, bottom),
        radius=18,
        fill=(220, 196, 151, 252),
        outline=BLACK,
        width=9,
    )
    draw.rounded_rectangle(
    ...
    draw.line((left + 31, top + 23, right - 31, top + 23), fill=INK, width=4)
    draw.line((left + 31, top + 29, right - 31, top + 29), fill=CRIMSON, width=2)

    # Black thorn stems and crimson roses turn the field group into one piece of
    # campaign art rather than a plain replacement table.
    for side in (1, -1):
        rose_x = left + 31 if side == 1 else right - 31
        draw_rose(draw, rose_x, top + 29, 21)
        stem_end = rose_x + side * 116
        draw.line((rose_x, top + 29, stem_end, top + 29), fill=INK, width=5)
        for offset in (35, 72, 105):
            thorn_x = rose_x + side * offset
            draw.line(
                (thorn_x, top + 29, thorn_x - side * 10, top + 18),
                fill=CRIMSON,
                width=4,
            )

    label_font = font(FONT_SERIF_BOLD, 17)
    value_font = font(FONT_SERIF_BOLD, 31)
    small_value_font = font(FONT_SERIF_BOLD, 25)
    name_font = font(FONT_GOTHIC, 72)
    ...
    def field(
        ...
        *,
        ...
        base_size = 25 if small else 31
        chosen_font = small_value_font if small else value_font
        available_width = x2 - x1
        while (
            draw.textbbox((0, 0), value, font=chosen_font)[2] > available_width
            and base_size > 16
        ):
            base_size -= 1
            chosen_font = font(FONT_SERIF_BOLD, base_size)
        draw.text((x1, y), value, font=chosen_font, fill=INK)
        line_y = y + 39
        draw.line((x1, line_y, x2, line_y), fill=BLACK, width=3)
        draw.line((x1, line_y + 4, x2, line_y + 4), fill=CRIMSON, width=2)
        draw.text((x1, line_y + 6), label.upper(), font=label_font, fill=CRIMSON)

    # Name, alignment, and player occupy the complete first row.
    name_x = left + 48
    name_y = top + 35
    draw.text(
        (name_x + 4, name_y + 4),
    ...
    name_rule_y = top + 112
    draw.line((name_x, name_rule_y, 1210, name_rule_y), fill=BLACK, width=4)
    draw.line((name_x, name_rule_y + 5, 1210, name_rule_y + 5), fill=CRIMSON, width=2)
    draw.text(
        (name_x, name_rule_y + 8),
        "CHARACTER NAME",
        font=label_font,
        fill=CRIMSON,
    )
    field(1250, 1570, top + 58, "Alignment", "Neutral Good")
    field(1610, right - 47, top + 58, "Player", "-")

    # Class, deity, and homeland form the second row.
    field(left + 48, 900, top + 146, "Class & Level", "Oracle (Spirit Guide) 17")
    field(940, 1275, top + 146, "Deity", "Hermes")
    field(1315, right - 47, top + 146, "Homeland", "-")

    # The final row deliberately runs all the way to Eyes at the right edge.
    third_y = top + 210
    field(left + 48, 720, third_y, "Race", "Old Mythic Advanced Suzerain Agathion-Blooded Aasimar", small=True)
    field(745, 890, third_y, "Size", "Medium", small=True)
    field(915, 1065, third_y, "Gender", "Male", small=True)
    field(1090, 1185, third_y, "Age", "34", small=True)
    field(1210, 1360, third_y, "Height", "5' 11\"", small=True)
    field(1385, 1555, third_y, "Weight", "180 lb.", small=True)
    field(1580, 1735, third_y, "Hair", "Black", small=True)
    field(1760, right - 47, third_y, "Eyes", "Blue", small=True)
