...
def draw_first_page_identity_banner(page: Image.Image, page_number: int) -> None:
    ...
    draw.line((campaign_x, rule_y, campaign_x + campaign_width + 250, rule_y), fill=BLACK, width=4)
    draw.line((campaign_x, rule_y + 5, campaign_x + campaign_width + 250, rule_y + 5), fill=CRIMSON, width=2)
    for offset in (campaign_width + 45, campaign_width + 125, campaign_width + 205):
        x = campaign_x + offset
        draw.polygon(
            ((x, rule_y - 7), (x + 7, rule_y), (x, rule_y + 7), (x - 7, rule_y)),
            fill=CRIMSON,
        )

    name_font = font(FONT_GOTHIC, 78)
    name_x, name_y = left + 48, top + 70
    draw.text(
        (name_x + 3, name_y + 3),
        "Demidius Thorne",
        font=name_font,
    ...
    draw.text((title_x, top + 170), title, font=title_font, fill=INK)
    draw.line((name_x, top + 214, split - 40, top + 214), fill=INK, width=4)
    draw.line((name_x, top + 220, split - 40, top + 220), fill=CRIMSON, width=2)
    subtitle_font = font(FONT_SERIF_BOLD, 19)
    draw.text((name_x, top + 247), "MYTHIC ORACLE  |  BLESSED OF HERMES", font=subtitle_font, fill=CRIMSON)
