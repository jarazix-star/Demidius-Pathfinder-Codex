...
def draw_first_page_identity_banner(page: Image.Image, page_number: int) -> None:
    ...
    # Keep the initial T clear of the upper-left corner ornament.
    campaign_x, campaign_y = left + 72, top + 18
    ...
    def header_filigree(center_x: int, center_y: int, width: int) -> None:
        ornament = Image.open(BOTTOM_CENTER_ART).convert("RGBA")
        target_height = 34
        ornament = ornament.resize(
            (width, target_height),
            Image.Resampling.LANCZOS,
        )
        page.alpha_composite(
            ornament,
            (center_x - width // 2, center_y - target_height // 2),
        )
    ...
    draw.text((name_x, top + 275), "MYTHIC ORACLE  |  BLESSED OF HERMES", font=subtitle_font, fill=CRIMSON)
