...
required = [
    "name", "type", "rating", "recommendation", "verification_status",
    "source", "publisher", "pdf_page", "citation", "tags", "summary", "gm_review"
]
...
for i, r in enumerate(rows, 2):
    ...
    if key in seen:
        errors.append(f"row {i}: duplicate name {r['name']}")
    seen.add(key)
if errors:
    print("\n".join(errors))
    sys.exit(1)
